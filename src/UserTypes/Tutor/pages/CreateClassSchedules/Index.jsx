...
const Index = () => {
  ...
  useEffect(() => {
    axios.get("https://localhost:7153/api/TeacherAuth/GetAll")
    .then(res => {
        setTeacher(res.data)
    })
  },[])
  ...
  useEffect(() => {
    if(selectTeacher){
        axios.get(`https://localhost:7153/api/TeacherAuth/GetById/${selectTeacher}`)
    .then(res => {
  ...
  useEffect(() => {
    axios.get("https://localhost:7153/api/Rooms",{
        headers: {
  ...
  useEffect(() => {
    axios.get("https://localhost:7153/api/ClassTime",{
        headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${user.token}`,
          },
        ...
        <form className="w-50 m-auto mt-5" onSubmit={(e) => handleSubmit(e)}>
          ...
          <div className="error-messages">
                      <p className="error-message" style={{color:"red"}}>
                        {error && (error.includes("This") || error.includes("written")) ? error : ""}
                      </p>
                    </div>
          ...
          {/* ---- */}
            ...
            >
                ...
                {teacher && teacher.filter(t => t.isDeleted === false).map(e => {
                    return (
          ...
          <div className="form-group">
            ...
            >
                ...
                {classTime && classTime.map(e => {
                    return(
                        <option key={e.id} value={e.id}>{e.startTime} : {e.endTime}</option>
                    )
                })}
            </select>
          ...
          <div className="form-group">
            ...
            >
                ...
                {room && room.map(e => {
                    return (
                        <option key={e.id} value={e.id}>{e.roomNumber}</option>
                    )
                })}
            </select>
