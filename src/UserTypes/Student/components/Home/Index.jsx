...
function Index() {
  ...
  useEffect(() => {
    axios.get(`https://localhost:7153/api/StudentAuth/GetByUserName?userName=${user.username}`, {
      headers: {
  ...
  return (
    <div className='py-2 student_home' style={{ width: "90%" }}>
      <FullCalendar
        ...
        selectOverlap={true}
        events={student && student.map(s => ({
          date: s.scheduleDate.substring(0, 10),
          title: s.room.roomNumber + " " + s.lesson.name + " " + s.classTime.startTime + " " + s.classTime.endTime,
        }))}
